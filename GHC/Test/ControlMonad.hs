...
reverseEvents :: FinalState stmt exp a -> FinalState stmt exp a
...
newtype BitConsuming stmt exp a =
    BC { unBC :: [Bool] -> [Event stmt exp] -> (FinalState stmt exp a, [Bool]) }
...
runWithBits :: BitConsuming stmt exp a -> [Bool] -> FinalState stmt exp a
-- ^ Run with Booleans determining decisions, return final
-- state with most recent event first
runWithBits m bits = fst $ unBC m bits []

instance MonadFail (BitConsuming stmt exp) where
  fail msg = BC $ \bits past -> (Failed past msg, bits)



instance ControlTestMonad stmt exp (BitConsuming stmt exp) where
  evalPredicate lbl =
